    ...
    def solve(self):
        ...
        if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
            ...
            for (d, n), p in solution.items():
                ...
                print(f"On day {d}, nurse {n} va trata: {len(p)}.")
                self.data.append(len(p))
            print()
...
def main():
    ...
    problem.solve()
    d = problem.data
    while len(d)< num_nurses:
        d.append(0)

    print("AOLUEEEEEEEEEEEEEE!!!!!!!")
    print(d)
    print("Media ", np.mean(d))
    variance_patients = np.var(d)
    std_dev = np.std(d)
    print("STD",std_dev)
    print("VAR", variance_patients)
    print("TIMP", (datetime.now()-start).total_seconds())
